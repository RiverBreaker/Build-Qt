...
def is_license_file(file_path):
    ...
    BINARY_EXTENSIONS = {'.png', '.jpg', '.gif', '.exe', '.dll', '.so', '.a', '.o', '.obj', '.pyc', '.class'}
    SOURCE_EXTENSIONS = {'.cpp', '.c', '.h', '.hpp', '.cc', '.cxx', '.m', '.mm', '.java', '.cs'}
    BUILD_EXTENSIONS = {'.cmake', '.pro', '.pri', '.qbs'}
    ...
    if file_path.suffix.lower() in BINARY_EXTENSIONS or \
       file_path.suffix.lower() in SOURCE_EXTENSIONS or \
       file_path.suffix.lower() in BUILD_EXTENSIONS:
        return False
